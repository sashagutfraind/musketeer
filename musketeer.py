...
"  url    = {https://ieeexplore.ieee.org/abstract/document/7266557/},",
"  doi    = {},",
...
from musketeer import graphutils
...
def export_visualization(new_G, write_graph, visualizer, output_path, verbose=True):
    image_path  = output_path + '.pdf'
    stderr_path = output_path + '.err.txt'

    try:
        #note: these commands would require pygraphviz and sfdp
        ...
        print('Error while writing and/or visualizing graph:')
...
def initialize():
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'M:chf:t:v:s:Tp:o:v:w:', 
        ['help', 'citation', 'input_path=', 'output_path=', 'graph_type', 
        'seed=', 'test', 'params=', 'visualizer=', 'verbose=', 'metrics:'])
    ...
    ret = {'params':params, 'input_path':input_path, 'graph_type':graph_type, 
            'visualizer':output_path, 'output_path':output_path, 
            'visualizer':visualizer, 'verbose':verbose, 
            'compare_replica':compare_replica, 'write_graph':write_graph}
    return ret
...
def input_prompter():
    ...
    try:
        ...
        print('Error parsing edge or node change rates!')
        print()
        sys.exit(1)
...
#wishlist: make this __main__ fit a page
if __name__ == '__main__':
    ...
    #wishlist: make fewer - too many reports
    ...
    if output_path == None:
        ...
        output_path = output_base + '__' + t_str + '.pkl'
        
        graphutils.safe_pickle(path=output_path, data=new_G, params=params)
        ...
        writedir = os.path.dirname(output_path)
        if writedir!='' and (not os.path.isdir(writedir)):
            raise ValueError(f'Target output folder ({writedir}) does not exist')
        sys.stdout.flush()
        graphutils.write_graph(new_G, output_path)
    ...
    export_visualization(new_G, write_graph, visualizer, output_path)
